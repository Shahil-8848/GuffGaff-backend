...
function setupTextChatServer(server) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("find-match", async () => {
      ...
      const waitingPartnerId = connectionManager.waitingQueue[0];
      if (waitingPartnerId && waitingPartnerId !== socket.id) {
        connectionManager.removeFromWaitingQueue(waitingPartnerId);
